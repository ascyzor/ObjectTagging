...
const initFileHandler = (scene: Scene, events: Events, dropTarget: HTMLElement) => {
    ...
    const importFiles = async (files: ImportFile[], animationFrame = false) => {
        ...
        if (isPlySequence(filenames)) {
            ...
            // If object_centroids.json is included in the batch, queue it for
            // automatic label loading once the splat finishes loading.
            const centroidFile = files.find(f => f.filename.toLowerCase() === 'object_centroids.json');
            if (centroidFile?.contents) {
                events.fire('centroids.pendingJson', centroidFile.contents);
            }
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                } else if (filename.endsWith('.json') && filename !== 'object_centroids.json') {
                    // load inria camera poses (object_centroids.json is handled separately above)
